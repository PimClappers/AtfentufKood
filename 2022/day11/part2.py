    ...
    if (line != ""):
        if (line.split()[0] == "Monkey"):
            ...
            for item in items:
                if (item[-1] == ","):
                    item = item[0: -1]
                monkeys[-1][ITEMS].append([0, int(item)])
...
while round < 10000:
    for monkey in monkeys:
        for item in monkey[ITEMS]:
            itemValue = item[1] + (item[0]*1000000000)
            if (monkey[OPERATOR] == "+"):
                if (monkey[FACTOR] != "old"):
                    itemValue = itemValue + int(monkey[FACTOR])
                else:
            ...
            if (monkey[OPERATOR] == "*"):
                if (monkey[FACTOR] != "old"):
                    ...
                    itemValue = itemValue * itemValue
            item[0] = int(itemValue / 1000000000)
            item[1] = itemValue % 1000000000
            print(item[1])
            if (itemValue % monkey[MODULO] == 0):
